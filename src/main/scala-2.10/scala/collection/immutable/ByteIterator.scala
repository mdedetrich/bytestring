...
import Collections.EmptyImmutableSeq

object ByteIterator {
  object ByteArrayIterator {

    protected[immutable] def apply(array: Array[Byte]): ByteArrayIterator =
      new ByteArrayIterator(array, 0, array.length)

    protected[immutable] def apply(array: Array[Byte], from: Int, until: Int): ByteArrayIterator =
  ...
  class ByteArrayIterator private (
      private[immutable] var array: Array[Byte],
      private[immutable] var from: Int,
      private[immutable] var until: Int
  ...
  object MultiByteArrayIterator {
    ...
    protected[immutable] def apply(iterators: LinearSeq[ByteArrayIterator]): MultiByteArrayIterator =
  ...
  class MultiByteArrayIterator private (private var iterators: LinearSeq[ByteArrayIterator]) extends ByteIterator {
    ...
    final override def length: Int = {
      val result = len
      iterators = MultiByteArrayIterator.empty.iterators // Manually inline clear() due to Scala 2.10.x limits
      result
    }

    private[immutable] def ++:(that: ByteArrayIterator): this.type = {
        ...
        if (that.isEmpty) this
          ...
          that match {
            case that: ByteArrayIterator =>
              ...
              that.array = Array.emptyByteArray; that.from = 0;
              that.until = that.from // Manually inline clear() due to Scala 2.10.x limits
              this
            case that: MultiByteArrayIterator =>
              iterators = this.iterators ++ that.iterators
              that.iterators =
                MultiByteArrayIterator.empty.iterators // Manually inline clear() due to Scala 2.10.x limits
              this
          }
        }
      case _ => super.++(that)
...
abstract class ByteIterator extends BufferedIterator[Byte] {
  ...
  private[immutable] def indexWhere(p: Byte => Boolean, from: Int): Int = {
    var index = 0
    while (index < from) {
      next()
      index += 1
  ...
  override def indexOf[B >: Byte](elem: B): Int = indexWhere(_ == elem, 0)

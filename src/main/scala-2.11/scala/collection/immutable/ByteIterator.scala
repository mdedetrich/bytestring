...
object ByteIterator {
  ...
  class MultiByteArrayIterator private (private var iterators: LinearSeq[ByteArrayIterator]) extends ByteIterator {
    ...
    final override def length: Int = {
      val result = len
      iterators = MultiByteArrayIterator.empty.iterators // Manually inline due to 2.10 inliner limits
      result
    }
        ...
        if (that.isEmpty) this
          ...
          that match {
            case that: ByteArrayIterator =>
              ...
              that.until = that.from // Manually inline due to 2.10 inliner limits
              this
            case that: MultiByteArrayIterator =>
              iterators = this.iterators ++ that.iterators
              that.iterators = MultiByteArrayIterator.empty.iterators // Manually inline due to 2.10 inliner limits
              this
          }
        }
      case _ => super.++(that)

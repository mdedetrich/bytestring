...
class ByteStringSpec extends AnyWordSpec with Matchers with Checkers {
  ...
  def likeVecIts(
      a: ByteString,
      b: ByteString
  )(body: (BufferedIterator[Byte], BufferedIterator[Byte]) => Any, strict: Boolean = true): Boolean = {
  ...
  "A ByteString" must {
    ...
    "behave as expected" when {
      ...
      "asByteBuffers" in {
        ...
        check { (a: ByteString) =>
          import ccompat.JavaConverters._
    ...
    // TODO: This test seems to be redundant now, remove?
    "serialize correctly" when {
      ...
      if (util.Properties.versionNumberString.startsWith("2.12")) {
        "parsing regular ByteString1C as compat" in {
          val oldSerd =
            ""
